...
#region DebugDrawFunctions
	    ...
	    /// </param>
	    /// <param name="segments"> Level of detail, segments per circle, minimum 4</param>
	    /// <param name='duration'>
            ...
            bool depthTest = true)
        {
            ...
            float angle = MathUtils.TwoPi / segments;
            ...
            {
                var new_x = new Vector3(position.x, position.y + radius * Mathf.Sin(angle * i),
                                        position.z + radius * Mathf.Cos(angle * i));
                var new_y = new Vector3(position.x + radius * Mathf.Cos(angle * i), position.y,
                                        position.z + radius * Mathf.Sin(angle * i));
                var new_z = new Vector3(position.x + radius * Mathf.Cos(angle * i), position.y + radius * Mathf.Sin(angle * i),
                                        position.z);
	    ...
	    public static void DebugWireSphere(
            ...
            bool depthTest = true)
        {
            DebugWireSphere(position, Color.white, radius, segments, duration, depthTest);
        }
            ...
            bool depthTest = true)
        {
            ...
            Vector3 interpolatedVector = Vector3.Slerp(_forward, _up, angle / 90.0f);

            var farPlane = new Plane(-direction, position + _forward);
            var distRay = new Ray(position, interpolatedVector);

            farPlane.Raycast(distRay, out float dist);

            Debug.DrawRay(position, interpolatedVector.normalized * dist, color);
            ...
            DebugCircle(position + _forward, direction, color, (_forward - interpolatedVector.normalized * dist).magnitude, duration, depthTest);
            DebugCircle(position + _forward * 0.5f, direction, color, (_forward * 0.5f - interpolatedVector.normalized * (dist * 0.5f)).magnitude, duration,
                        depthTest);
        }
            ...
            bool depthTest = true)
        {
            ...
            for (var i = 1; i < 26; i++)
            {
                ...
                Debug.DrawLine(Vector3.Slerp(-right, -up, i / 25.0f) + start, Vector3.Slerp(-right, -up, (i - 1) / 25.0f) + start, color, duration, depthTest);
                Debug.DrawLine(Vector3.Slerp(forward, -up, i / 25.0f) + start, Vector3.Slerp(forward, -up, (i - 1) / 25.0f) + start, color, duration,
                               depthTest);
                Debug.DrawLine(Vector3.Slerp(-forward, -up, i / 25.0f) + start, Vector3.Slerp(-forward, -up, (i - 1) / 25.0f) + start, color, duration,
                               depthTest);
...
#region GizmoDrawFunctions
        ...
        public static void DrawCone(Vector3 position, Vector3 direction, Color color, float angle = 45)
        {
            ...
            Gizmos.DrawRay(position, interpolatedVector.normalized * dist);
            ...
            DrawCircle(position + _forward, direction, color, (_forward - interpolatedVector.normalized * dist).magnitude);
            DrawCircle(position + _forward * 0.5f, direction, color, (_forward * 0.5f - interpolatedVector.normalized * (dist * 0.5f)).magnitude);
